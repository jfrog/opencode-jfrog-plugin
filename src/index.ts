...
import { appendFileSync, createWriteStream, readFileSync, writeFileSync, existsSync, mkdirSync } from "fs";
...
const INSTRUCTIONS_REGISTRY_URL = "https://releases.jfrog.io/artifactory/run/ai/integrations/opencode/JFROG-INTEGRATION-MANAGEMENT.md";

const fetchAndSaveFile = async (url: string, destPath: string, log: (msg: string) => void): Promise<{ success: boolean; error?: string }> => {
...
const extractZip = async ($: any, directory: any,  skillZipFile: string, skillName: string, skillZipDir: string, log: (msg: string) => void): Promise<{ success: boolean; error?: string }> => {
    // extract zip file, and check command response
    const unzipResponse = await $`unzip -o ${skillZipFile} -d ${directory}/.opencode/skills/${skillName}`.nothrow().quiet();
    if (unzipResponse.exitCode !== 0) {
        log(`Failed to extract JFrog ${skillName} skill: ${unzipResponse.stderr}`);
        return {  success: false, error: `Failed to extract JFrog ${skillName} skill: ${unzipResponse.stderr}`};
    }
...
const setupPackageManagers = async (client: any, $: any, directory: any, sessionId: any, log: (msg: string) => { success: boolean; message: string } | undefined) => {
    ...
    try {
        ...
        return { success: false, message: "Jfrog cli is not installed, please use the jfrog-cli skill to install it" };
    }
    ...
    if (!existsSync(packageManagersFile)) {
        return { success: false, message: "Jfrog packages are not setup, please use the jfrog-setup-package-managers skill to complete setup. type /skill and select jfrog-setup-package-managers" };
    ...
    if (!configuredPackageManagers) {
        return { success: false, message: "Jfrog packages are not setup, please use the jfrog-setup-package-managers skill to complete setup. type /skill and select jfrog-cli" };
    }

    const results: { success: { packageManager: string }[]; error: { packageManager: string; error: string }[] } = { success: [], error: [] };
...
const pullSkills = async( $, directory, log: (msg: string) => void): Promise<{ success: boolean; failedSkills?: string[] }> => {
    ...
    for (const skill of skillsToPull) {
        const skillExists = await $`test -d ${directory}/.opencode/skills/${skill.name}/`.nothrow().quiet()
        if (skillExists.exitCode !== 0) {
            ...
            if (!result.success) {
                ...
                const unzipResult = await extractZip($, directory, skillZipFile, skillName, skillZipDir, log);
...
const jfrogOpencodePlugin: Plugin = async ({ project, client, $, directory, worktree }) => {
   ...
   // TODO add user message if skills are not imported, after checking !pullSkillsResponse.success
